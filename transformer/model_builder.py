...
class TransformerModelBuilder:
    ...
    def build_transformer_model_with_pid(self, input_shape, pid_vocab_size, forecast_horizon, target_len, pid_embedding_dim=10, head_size=64, num_heads=4, ff_dim=128, num_transformer_blocks=4, dropout=0.1):
    ...
    def weighted_mae_loss(self,y_true, y_pred,weight=[1,5,1]):
        """
        weight:[1,2,1] 代表每个target的权重
        """
        weights = tf.constant(weight, dtype=tf.float32)
        mae_per_col = tf.reduce_mean(tf.abs(y_true - y_pred), axis=0)
        weighted_loss = tf.reduce_sum(mae_per_col * weights) / tf.reduce_sum(weights)
